...
lightLine.style.cssText +=
  ";height: 1px; position: absolute; top: 0; left: 0; right: 0; background-color: skyblue; z-index: 999; display: none";
export function useTableDragSort() {
  ...
  function initDragSort(tableBodyDom, tableData) {
    setTimeout(() => {
      ...
      const dragEnterTextList =
        tableBodyDom.querySelectorAll("tr .dragEnterText");
      ...
      for (let i = 0; i < trList.length; i++) {
        moveIconDoms[i].onmousedown = () => {
          console.log("moveIconDoms", moveIconDoms);
          trList[i].setAttribute("draggable", "true");
          console.log("ok", trList[i]);
          currentDragDom = trList[i];
          console.log("currentDragDom", currentDragDom);
          currentDragIndex = i;
        };
        ...
        trList[i].ondragenter = (e) => {
          ...
          const top = Math.max(
            Math.abs(Math.round(e.offsetY / trHeight)) * trHeight +
              trList[i].offsetTop -
              1,
            0
          );
